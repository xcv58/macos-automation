...
| Physical App Store source eject | In the exact signed sandboxed MAS build, verify the named eject action before scan, after scan, and on a successful receipt, then invoke it from the receipt | Passed 2026-08-31. All three controls appeared only for the selected verified removable source. Receipt ejection unmounted the physical Secure Digital card, changed the receipt to `SD Card ejected. Safe to remove.`, and required no temporary sandbox exception. A follow-up reinsertion exposed a race where the helper prompt could precede `/Volumes` enumeration; refreshing the snapshot after consent fixed it, and the first and review controls were reverified without manual refresh. |
