...
| --- | --- | --- |
| Public App Store support and privacy pages | The canonical marketing, support, and privacy URLs load over HTTPS and contain the current App Store disclosures; legacy URLs embedded in Build 4 remain usable | Passed 2026-09-03. The updated static site was deployed to the existing Vercel project and `sd.jenny.media` was attached and verified. `https://sd.jenny.media/`, `/privacy.html`, and `/support.html` returned HTTP 200. The legacy `sd-import.jenny.media` and `macos-automation.vercel.app` URLs also returned HTTP 200, preserving Build 4's embedded support and privacy links. The first-party support page uses `sd@jenny.media`; the owner confirmed that mailbox works, and GitHub Issues remains a secondary public bug-report path rather than the only support destination. A replacement light/dark capture set and 16.5-second H.264 workflow preview are prepared locally from window-only Build 4 captures with an 8:5 presentation; unlike the currently deployed set, they contain neither the remote-control indicator nor an embedded black canvas. A local Chrome preview loaded both themes and every new image, stylesheet, and video asset with HTTP 200; public redeployment verification remains pending. |
| App Store screenshot asset capture | Produce accepted-size images without personal media or the prohibited source, including a purchase image with localized price and a clean completed-import receipt | Partially passed 2026-09-03. Five opaque 2560 x 1600 PNGs are prepared: exact-TestFlight-Build-4 synthetic file preview, destination plan, successful receipt reporting 2 copied, 0 skipped, and 0 failed, and Settings showing `Background helper: Running` plus `Lifetime access unlocked`; the fifth image is the real purchase sheet from the checked-in local StoreKit configuration with its localized `$9.99` price and Family Sharing disclosure. The Build 4 plan, file, receipt, and Settings images were captured window-only, so they contain neither the remote-control indicator nor a black canvas. The purchase capture was reframed to 16:10 and only the external remote-control indicator in the otherwise blank titlebar area was removed; product UI was not altered. Both imported synthetic JPEGs matched their sources byte-for-byte by SHA-256. The purchase sheet is not a processed-TestFlight purchase capture because the TestFlight account already owns the product. The consent prompt and a new named-eject receipt image remain blocked until an allowed removable card is available; `Sandisk 4T` was not used. |
...
| Physical App Store card and helper | Quit the main app, insert a removable card, verify the helper wakes it without enumeration, decline consent with no scan, then accept consent and reuse or refresh folder authorization | Passed on TestFlight Build 3 on 2026-09-01 with the removable ExFAT Secure Digital volume `Untitled`; `Sandisk 4T` was not used. With the main app quit, insertion woke Build 3 and showed `SD Import detected this removable volume but has not scanned its contents.` Choosing `Don't Scan` returned to the source screen without a scan or review. After authorizing an isolated synthetic-only subfolder, another quit/reinsert produced the same consent sheet; `Allow Scan` automatically selected that persisted source on `Untitled`, scanned exactly one synthetic JPEG, and later reported `Nothing New`. A separately authorized temporary destination received 1 copied, 0 skipped, and 0 failed. The 24,073-byte source and copy matched byte-for-byte with SHA-256 `8cfb68e6de30269d29287fddd4c1743d3550c50422b9aa92b89fae64bff32b71`. No existing card media was imported or changed. |
| Physical App Store source eject | In the exact signed sandboxed MAS build, verify the named eject action before scan, after scan, and on a successful receipt, then invoke it from the receipt | Passed on the signed MAS Debug build on 2026-08-31 and TestFlight Build 3 on 2026-09-01. In Build 3, Eject was available for `Untitled` before scanning, in the one-file review before import, on the successful receipt, and after the `Nothing New` rescan. Invoking it removed `/Volumes/Untitled`; `diskutil` could no longer find the whole Secure Digital device `disk10`, and the app reported `Source ejected safely` on the source-phase pass. This card exposed one volume, so the multi-volume grouped-eject behavior was not newly exercised; the prior grouped DJI hardware pass remains the applicable evidence. |
