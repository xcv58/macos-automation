...
| --- | --- | --- |
| Public App Store support and privacy pages | The canonical marketing, support, and privacy URLs load over HTTPS and contain the current App Store disclosures; legacy URLs embedded in Build 4 remain usable | Passed 2026-09-03. The updated static site was deployed to the existing Vercel project and `sd.jenny.media` was attached and verified. `https://sd.jenny.media/`, `/privacy.html`, and `/support.html` returned HTTP 200. The legacy `sd-import.jenny.media` and `macos-automation.vercel.app` URLs also returned HTTP 200, preserving Build 4's embedded support and privacy links. The first-party support page uses `sd@jenny.media`; GitHub Issues remains a secondary public bug-report path rather than the only support destination. The hero, gallery, posters, and 16.5-second H.264 workflow preview were refreshed from the clean synthetic Build 4 capture set; the deployed DOM exposes the new media, updated alt text, four-step transcript, and 0:16 duration without the prior DJI filenames or personal-looking source and destination paths. |
| App Store screenshot asset capture | Produce accepted-size images without personal media or the prohibited source, including a purchase image with localized price and a clean completed-import receipt | Partially passed 2026-09-03. Four 2560 x 1600 PNGs were prepared: synthetic file preview, destination plan, lifetime purchase sheet, and a successful receipt reporting 2 copied, 0 skipped, and 0 failed. Both imported synthetic JPEGs matched their sources byte-for-byte by SHA-256. A supplemental exact-TestFlight-Build-4 Settings image shows `Background helper: Running` and `Lifetime access unlocked`. The purchase sheet is a real app UI capture from the checked-in local StoreKit configuration, not a processed-TestFlight purchase capture, because the TestFlight account already owns the product. The consent prompt and a new named-eject receipt image remain blocked until an allowed removable card is available; `Sandisk 4T` was not used. |
