...
| TestFlight lifetime purchase persistence | Complete the no-charge TestFlight purchase, verify unlimited access, quit and relaunch, then verify the entitlement is restored without waiting for product metadata; Restore Purchases must return promptly and must not authenticate when the verified entitlement is already available | Passed on TestFlight Builds 2 and 3 on 2026-09-01. Build 2 first proved the existing Build 1 purchase appeared immediately, Restore Purchases returned in about 1.4 seconds without authentication, and cold-relaunch recovery took about 1.1 seconds. In the installed TestFlight Build 3, Settings showed `Lifetime access unlocked` immediately without waiting for product metadata. Restore Purchases returned to its enabled state in 689 ms without an authentication sheet. In a timed cold cycle, the main window appeared in 884 ms and Settings showed the restored unlock by 2.090 seconds after the launch request, including navigation to Settings. Build 3 contains the stale-latest-transaction revocation guard, was compiled with accepted Xcode 26.6, distribution-signed with the exact profiles, passed strict archive verification, uploaded successfully, reached `Complete` / `Ready to Submit`, and was assigned to `IQ Internal QA`. Final automation passes 7 deterministic commerce tests and 6 app-hosted StoreKit tests. This active-entitlement pass did not put the live transaction into a revoked state; live sandbox Family Sharing grant/revoke QA remains. |
