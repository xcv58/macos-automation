...
| Synthetic helper handoff and decline | Register the real embedded helper, inject one event after removable-volume eligibility, persist it through the App Group mailbox, wake the containing app, verify the explicit no-scan consent sheet, choose `Don't Scan`, and observe no folder or review UI | Scripted gate is ready. Expected-failure pass on 2026-08-31 rejected `Mac Team Provisioning Profile: *` before installation because it lacks the exact App IDs and App Group; exact app/helper Apple Development profiles are required. This gate deliberately does not cover OS mount detection because disk images are rejected by production. |
